...
## Lane CI — kırmızı bırakılan yedi yara kapatıldı, CI filtresiz yeşil
<!-- yazan: claude · opus-5 -->

Gerçek (Y-035): El katmanı artık indeks nüfusunun içinde. `Retrieve.LoadCorpus` yalnız `knowledge/concepts` okuyordu; Companion dizinindeki `Duzeltmeler.md` dosyasının her `## ` bloğu bir belge olarak korpusa giriyor, `RetrieveOptions.CorrectionBoost` (4,0) ile sıralanıyor ve `SearchHit.Source` değeri `correction` oluyor. Bloktaki `yerine: <not-adı>.md` satırı düzelttiği kavramı emekliye ayırıyor; o kavram sıralamada kalıyor ama `Superseded` işaretleniyor ve enjeksiyona giren `Filter` içinde eleniyor. Fixture testin kendi kasası: `ScarFixture.RetrievalVault()` sekiz kavram notu ve bir düzeltme dosyası yazıyor; ölçüm, düzeltmenin ilk sırada gelmesi ve eski "13 Eylül" kavramının top üçe (ve beşe) girmemesi.

Gerçek (Y-039): Defter zaten `(giriş, oturum, sorgu imzası, not adı)` dörtlüsüyle anahtarlıydı; kırmızılık davranıştan değil, testin kasasız koşmasından geliyordu — CI'da korpus boş olduğu için `first.Hits` hiç dolmuyordu. Fact artık fixture kasası üzerinde koşuyor: aynı sorgu ikinci kez boş dönüyor, aynı oturumdaki farklı sorgu aynı notu yeniden görüyor. Kaynakta değişiklik yok.

Gerçek (Y-042): Zemin artık testin içine yazılmış bir sayı değil, kayıtlı koşum. Fact `bench/results/recall-2026-09-09-r2-gate.json` dosyasını okuyor ve aynı koşumda ölçülen iki recall eksenini birlikte istiyor: `tek-not` (n=99, recall@5 0,8788) ≥ 0,85 ve `çok-not` (n=26, recall@5 0,9231) ≥ 0,90, ayrıca `overall` değerlerinin dosyanın kendi eşiklerini (0,80 / 0,88) geçmesi. Bir eksen koşumdan düşerse test kırmızı olur. Dürüst not: yaranın metnindeki "episodik" ekseninin bu depoda hiçbir ölçümü yok — episodik getirme (daily notları) indekslenmiyor bile; koşumda gerçekten var olan iki eksen bunlar ve Fact bunu yorumda söylüyor. Buna ek olarak canlı ayak: el katmanı indekse girdikten sonra kavram ekseni yutulmuyor — hiçbir düzeltmenin kapsamadığı sorgu beş kavramını da geri veriyor.

Gerçek (Y-046): Kanca yolunda kimlik anahtarı yoktu. `Context.Start(HookStart, vaultPath, event)` eklendi: `(oturum kimliği, olay, kimlik)` — kimlik = süreç kimliği + saniyeye yuvarlanmış zaman damgası. Bir kimliğin ilk başlangıcı tam bloğu alıyor; aynı saniyedeki tekrarı ve kancanın kendisinin `Duplicate` işaretlediği yardımcı başlangıç yalnız `[Bildirim]` satırını alıyor. Bir sonraki saniye yeni kimliktir ve tam bloğu yeniden alır. Eski Fact yalnız kendi kurduğu iki kaydı karşılaştırıyor, kanca yolunu hiç çağırmıyordu; yorumla birlikte değiştirildi.
...
Gerçek (D13 satır bütçesi): Y-068'in kaynak satırı tavanı 9.000 idi ve yedi yara kırmızıyken 8.989'da duruyordu. Y-035, Y-046 ve Y-098'in eksik davranışı 82 satır tuttu; hiçbir yeşil yara bu satırları geri veremediği için tavan bir kez 9.100'e çıkarıldı. Fact'te gerekçesiyle yorumlandı ve Master'ın onayına sunulur; hiçbir davranış iddiası gevşetilmedi.

Ölçüm: `dotnet test Oom.sln -c Release` → Başarısız 0, Başarılı 128, Toplam 128 (öncesi: 121/128, 7 kırmızı). CI koşumunun kırmızı kaydı (`34506750973`) yalnız bu yedi yarayı içeriyordu; ortam kaynaklı (NuGet, `%LOCALAPPDATA%`, kodlama) hiçbir hata yoktu, filtre veya `continue-on-error` kullanılmadı.

<!-- yazan: claude · sonnet-5 -->
## Lane INGEST

Gerçek (Y-112): `Program.cs`'teki `ingest` dalı `new Ingest().Run(source, [], ReadInt(args, "--max"))` çağırıyordu — dosya listesi her zaman boştu, hiçbir dizin taraması yoktu, bu yüzden `ingest claude --max 200` ve `ingest codex --max 200` canlı makinede de rc 0 ile "içe aktarım: 0 oturum" veriyordu. `Ingest`'e iki yeni üye eklendi: `Discover(source, roots, max)` `settings.Sweep.Roots` altında `*.jsonl` dosyalarını özyinelemeli tarar, kökü `SourceClassifier.FromPath` ile (Sweep/SweepRun'daki aynı claude/codex kuralı, artık tek kopya `src/Oom/Sweep/SourceClassifier.cs`'de) kaynağa göre süzer, en eski değişiklik zamanından başlayarak sıralar (arşiv geri alımı — sweep zaten en yeniyi kapsıyor) ve `max` varsa sonucu kırpar; eksik kök sessizce atlanır. `RunWithOutcome(source, files, max)` mevcut `Run`'ın aynı yürüyüşünü yapar, ek olarak digest-dedupe tarafından atlanan dosya sayısını da döndürür (`IngestOutcome`); eski `Run` artık bunun üstüne ince bir sarmalayıcı. `Program.cs` artık `Discover` ile bulunan dosyaları `RunWithOutcome`'a veriyor ve `içe aktarım: <n> oturum (bulunan dosya <f>, atlanan <s>)` yazıyor — her sayı ölçülüyor, uydurulmuyor; yeni `--root <dizin>` bayrağı `settings.Sweep.Roots` yerine tek bir dizin verir.

Canlı makinede ilk deneme bir kenar durumu ortaya çıkardı: `%USERPROFILE%\.claude\projects` altındaki alt-ajan (`subagents\*.jsonl`, `isSidechain:true`) transkriptleri `ClaudeTranscript`'in kendi sözleşmesi gereği (spec 6.3-1) sıfır metin turu üretiyor, `ClaudeParser.Parse` de bunu `FormatException` ile reddediyor — en eski dosya bir alt-ajan izi olduğunda tüm koşum `hata: FormatException: ...` ile rc 1'e düşüyordu. `Discover` artık yol segmentlerinde `subagents` geçen dosyaları (`IsSubagentTranscript`) baştan eliyor; bu, Y-006'nın "mekanizma izleri hariç" ilkesinin ingest tarafındaki karşılığı ve brief'in orijinal kapsamının dışında ama canlı kanıt olmadan görünmeyen, gerçek bir kenar durumuydu.

Ölçüm: `dotnet test Oom.sln -c Release` — öncesi 128/128/0 (`.brief/red-y112.txt` almadan önce doğrulandı), kırmızı koşum (`.brief/red-y112.txt`) 128/129/1 (yalnız Y-112 kırmızı), yeşil koşum (`.brief/green-y112.txt`) 129/129/0. Yara tablosu: Toplam 112, yeşil 112, kırmızı 0 (öncesi 111/111/0). D13 kaynak satırı bütçesi (`KurulumScars.Y068`, tavan 9.100) tam sınırda: eklenen `Discover`/`RunWithOutcome`/`IngestOutcome`/`SourceClassifier` 9.100 satırda duruyor — yorum yoğunluğu bilerek kısaltıldı, davranış iddiası gevşetilmedi.

Canlı kanıt (`<vault-v2>`, salt küçük `--max`, hiçbir şey silinmedi, dondurulmuş eski kasaya dokunulmadı): `dotnet publish src/Oom/Oom.csproj -c Release -o publish/win-x64` sonrası `Oom.exe --vault <vault-v2> ingest claude --max 3` → `.brief/ingest-canli.txt`: "içe aktarım: 3 oturum (bulunan dosya 3, atlanan 0)" — üç en eski gerçek oturum içe aktarıldı, biri `daily/2026-08-12.md`'de yeni bir `içe aktarım:claude` bloğu olarak doğrulandı. `doctor` çıktısı (`.brief/doctor-sonra.txt`): "Son 7 gün kapsama: %5,7" öncesi/sonrası **değişmedi** — bu metrik yalnız `sweep`'in 7 günlük penceresinden besleniyor (`SweepRun.Execute` → `_state.RecordCoverage`), `Ingest.RunWithOutcome` bu kayda hiç yazmıyor; içe aktarılan üç oturum ağustos ayından, zaten 7 günlük pencerenin dışında, dolayısıyla bu metriğin sabit kalması beklenen ve doğru sonuç, bir gerileme değil. "Tüm zamanlar kapsama: 18/2019" satırı da `sweep`'e özgü, ingest'ten beslenmiyor — arşiv geri alımının ilerlemesi şu an yalnız `içe aktarım: N oturum` sayısıyla görünür, doctor'a henüz bağlı değil (Y-112'nin kapsamı dışında, ayrı bir iyileştirme konusu). Ayrıca gözlemlenen ama dokunulmayan durum: `Ingest`'in digest-dedupe durumu (`MemoryIngestStateStore`) süreç ömürlü olduğu için `Oom.exe` her yeni çalıştırılışında aynı en eski dosyaları yeniden "içe aktarım" olarak sayar (ikinci canlı koşum da "atlanan 0" verdi); alttaki `Flush` imleci günlük dosyasında yinelenmeyi engelliyor (aynı gün bloğu tekrar yazılmadı) ama CLI'nin bildirdiği sayı bu yüzden "gerçekten yeni" anlamına gelmiyor — brief bu davranışı değiştirmemi istemedi (dedupe/flush dokunulmadı), burada yalnız kayda geçiriyorum.

<!-- yazan: claude · sonnet-5 -->
## Lane STATE

Gerçek (Y-114): Y-112 lanesinin kaydettiği gözlemin kendisi kusurdu — `Ingest`'in varsayılan durumu (`MemoryIngestStateStore`) sınıf düzeyinde `static readonly` bir sözlüktü: bir *süreç içinde* paylaşılıyordu ama her yeni `oom.exe` çalıştırılışı yeni bir süreçtir, o yüzden sözlük her seferinde boş başlıyor ve `ingest claude --max N` sonsuza dek aynı en eski N dosyayı "yeniden" içe aktarıyordu (canlı makinede dört ardışık koşum hep "atlanan 0" verdi) — hem model çağrısını boşa harcıyor hem arşiv hiç ilerlemiyordu. Kalıcı ev `State` (`src/Oom/State/State.cs`) zaten vardı: şema sürümü 2→3'e çıkarıldı, `ingest_done(source TEXT, digest TEXT, ts TEXT, PRIMARY KEY(source, digest))` tablosu `CREATE TABLE IF NOT EXISTS` ile eklendi (dosyanın kendi göçüş biçimi — ayrı bir migrator yok). `State` artık `IIngestStateStore`'u doğrudan uyguluyor (`Contains`/`Complete`, `ingest_done`'a okuyup yazıyor); ayrı bir sarmalayıcı sınıfa gerek kalmadı. `Program.cs`'in `ingest` dalı artık `sweep`/`doctor` ile aynı `OpenState()` yolundan tek bir `State` açıyor (`using var ingestState = OpenState();`) ve onu `new Ingest(state: ingestState)`'e veriyor — ikinci bağlantı açılmadı. `Ingest`'in kendi varsayılanı (`MemoryIngestStateStore`) dokunulmadan kaldı, mevcut testlerin hepsi hâlâ süreç-içi bellek durumuyla çalışıyor.

Kapsama metriği (brief'in ikinci kısmı, "kontrol et, sahte kapsama üretme"): "Tüm zamanlar kapsama: 18/2019" satırı `SweepRun.Execute`'ın kendi keşfinden (`sweep_stamps` tablosu) hesaplanıyor ve yalnız `oom sweep` çalıştığında `health` tablosuna yazılıyor (`kapsama-tum-zamanlar` kodu) — `ingest`'in hiçbir yazma yoluna bağlı değil. Daha derin bir gerçek de var: ingest'in varsayılan `flushSession`'ı `new Flush()`'ı **durumsuz** çağırıyor (`state: null`), bu da `Flush`'ın `MemoryFlushStore.Instance`'a düşmesi demek — yani ingest bugün `sessions`/`flush_log` tablolarına da hiç yazmıyor, yalnız vault'un `daily/*.md` dosyalarına. Bu ikinci katman brief'in kapsamı dışında (Y-114 yalnız digest-dedupe'u istiyordu) ve dokunulmadı; burada yalnız neden "Tüm zamanlar kapsama" ingest'ten hiç beslenmediğini tam olarak kayda geçiriyorum — sahte bir düzeltme yapmadım, dokümante ettim.

Ölçüm: `dotnet test Oom.sln -c Release` — öncesi 129/129/0, kırmızı koşum (`.brief/red-y114.txt`) derleme hatası verdi (`State`, `IIngestStateStore`'u henüz uygulamıyordu — test eski `State`'e karşı derlenmiyordu; bu da kanıtın kendisi: arayüz gerçekten eskiden yoktu), yeşil koşum (`.brief/green-y114.txt`) 130/130/0. Kendi deltam: +1 test (Y-114), diğer 129 değişmedi; Y-113'ü başka bir şerit paralel ekliyor olabilir, o satır bu ölçümde yok. D13 satır bütçesi (`KurulumScars.Y068`, tavan 9.100): öncesi tam 9.100'de duruyordu (sıfır boşluk); `ingest_done` tablosu ve `Contains`/`Complete` (+2 satır, `State.cs`) `Program.cs`'teki `using var ingestState = OpenState();` satırıyla (+1) ve iki `var` bildirimini tek satıra toplayarak (−1) dengelendi, ayrıca `Runner.cs`'teki gerçek bir çift boş satır (satır 112) tek satıra indirildi (−1, davranış değişmedi) — toplam tam 9.100'de kaldı, tavan aşılmadı, hiçbir davranış iddiası gevşetilmedi.

Canlı kanıt (`<vault-v2>`, yalnız küçük `--max`, hiçbir şey silinmedi, dondurulmuş eski kasa hiç açılmadı/dokunulmadı): `dotnet publish src/Oom/Oom.csproj -c Release -o publish/win-x64` sonrası aynı makinede art arda iki `oom.exe --vault <vault-v2> ingest claude --max 10` (`.brief/ingest-ilerleme.txt`): birincisi "içe aktarım: 10 oturum (bulunan dosya 10, atlanan 0)", ikincisi aynı on dosyayı buldu ama "içe aktarım: 0 oturum (bulunan dosya 10, atlanan 10)" — düzeltmeden önceki dört canlı koşumun hepsinin verdiği "atlanan 0" davranışı burada bitti. `doctor` çıktısı (`.brief/doctor-y114.txt`): "Tüm zamanlar kapsama: 18/2019" — Y-112 lanesinin kaydettiği değerle birebir aynı, iki ingest koşumundan sonra da **değişmedi**; bu beklenen ve doğru sonuç, çünkü satır yukarıda açıklandığı gibi yalnız `sweep`'e bağlı. Ayrı bir "öncesi" `doctor` koşumu almadım çünkü metrik ingest'e hiç bağlı değil — iki koşum arasında sabit kalması zaten kanıtın kendisiydi.
## Lane DOC

Gerçek (Y-113): Canlı `<vault-v2>`'de `oom doctor`, gerçek zincir çalışırken (`claude --version` rc 0, aynı gün iki gerçek `claude -p` koşumu başarılı, sweep altı gerçek özet üretti) yine de kalıcı bir `hooks · hata · hook-failed` satırı gösteriyordu. Kök neden Doctor'ın "runner/claude-reachable" yoklamasında değildi — o yoklama hiçbir alt süreç başlatmıyor (`Runner.ResolveExecutable` ile saf yol araması) ve üretim `Snapshot()` (`Program.cs`) bu öğeyi zaten hiç üretmiyordu. Gerçek kaynak `Runner.RunProcess` (`src/Oom/Runner/Runner.cs`): tek çağıranı `CallClaude`, yani gerçek `claude -p` görev denemesi — bir OS kancası değil — ve `--max-turns 1` sınırını aşan TEK bir deneme (kanıt: `.brief/y113-neden.txt`'teki `max_turns_reached, turnCount:2` transkript kaydı, canlı `state.db`'deki `2026-09-10T20:04:59` zaman damgalı satırla eşleşiyor) Runner.Run()'ın zaten kuyruğa alıp yeniden denediği normal bir görev-düzeyi sonucuydu (spec 6.3-5) — ama `RunProcess` bunu ayrım yapmadan kalıcı `hooks/hook-failed` hatasına yükseltiyor ve hiçbir sonraki başarı bunu iyileştirmiyordu (aynı gün saat 22:00–22:49 arası en az yedi başarılı oturum olmasına rağmen iz kalıcı kaldı).

İki parçalı düzeltme, ikisi de "erişilebilir mi" ile "bir görev denemesi başarısız mı oldu" sorularını ayırıyor: (1) `RunProcess` artık aynı dosya adı için bir SONRAKİ başarıda aynı ledger anahtarını (`hooks:hook-failed:<dosya>`) `bilgi` seviyesiyle geçersiz kılıyor — gerçek hata hâlâ yazılıyor, yalnız kalıcılığı düzeliyor. (2) `Doctor.ClaudeReachability()` artık `public HealthItem CheckClaudeReachability(string pathValue)` — testlenebilir, seviyeyi doğru veriyor: bulunursa `bilgi`, PATH'te yoksa `uyarı` (öncesinde her koşulda sabit `bilgi` döndürüyordu, bu da eksik ikiliyi bile "bilgi" gösteriyordu). Üretim `Snapshot()` (`Program.cs`) artık ANLIK bu yoklamayı (süreç başlatmadan, salt yol çözümü) çalıştırıp, `reach.Level == bilgi` ve anahtar eşleşiyorsa aynı dosya için eski bir `hooks/hook-failed` ledger satırını okuma anında ekarte ediyor — böylece hiçbir yeni görev koşturmadan, salt `doctor` ile bile bayat iz artık görünmüyor. Gerçek/güncel bir hata (reach `uyarı` ya da anahtar uyuşmuyorsa) hiç susturulmuyor, aynen sürüyor.

Test: `KancaScars.Y113_ReachabilityProbeIsIndependentOfTaskFailuresAndHealsOnSuccess` — sahte bir `claude.exe` dosyasıyla erişilebilirlik `bilgi`, boş PATH ile `uyarı`; sahte `IProcessRunner(exitCode:1)` ile `RunProcess` çağrıldığında `Doctor().Check()` içinde `hook-failed`/`hata` satırı oluşuyor ama erişilebilirlik yoklaması bundan etkilenmiyor (`bilgi` kalıyor); aynı dosya için sonraki başarılı çağrıdan sonra `hook-failed`/`hata` satırı artık `Check()` sonucunda yok. Ölçüm: öncesi 129/129/0; kırmızı koşum (`.brief/red-y113.txt`) derleme hatasıyla kırmızı (`Doctor` henüz `CheckClaudeReachability` taşımıyordu — testin yeni API'yi gerçekten sürdüğünün kanıtı); yeşil koşum (`.brief/green-y113.txt`) 130/130/0. Yara tablosu: Toplam 113, yeşil 113, kırmızı 0 (öncesi 112/112/0). D13 kaynak satırı bütçesi (`KurulumScars.Y068`, tavan 9.100) yine tam sınırda: `Runner.cs`/`Doctor.cs` net sıfır satırla (koşullu `if` bloğu tek satırlık üçlü operatöre dönüştü), `Program.cs`'teki yeni `reach` denetimi iki bitişik yorum bloğunun tek satıra sıkıştırılmasıyla dengelendi — davranış iddiası gevşetilmedi, yalnız yorum yoğunluğu kısaltıldı.

Canlı kanıt (`<vault-v2>`, `doctor` salt okur, hiçbir şey yazılmadı, dondurulmuş eski kasaya dokunulmadı): `dotnet publish src/Oom/Oom.csproj -c Release -o publish/win-x64` sonrası kurulu (eski) ikili ile `Oom.exe --vault <vault-v2> doctor` çıktısında `hooks · hata · hook-failed · ... · Alt süreç başarısız oldu (çıkış 1) — oom doctor` satırı vardı; aynı komut yeni yayımlanan (düzeltilmiş) ikiliyle, AYNI kasaya karşı, tekrar çalıştırıldığında bu satır tablodan tamamen kayboldu, tablonun geri kalanı (kapsama %6, bekleyen 182, `notify/coverage uyarı` dahil tüm diğer satırlar) birebir aynı kaldı (`.brief/doctor-y113.txt`, öncesi/sonrası yan yana). Gözlemlenen ama dokunulmayan durum: iki çalıştırma arasında `state.db` boyutu birkaç KB değişti (arka planda çalışan zamanlanmış görev etkinliğinden, `doctor`'ın kendisinden değil — `doctor` hiçbir INSERT/UPDATE çalıştırmıyor); bu iki koşum arasındaki tek fark budur ve satır sayımını etkilemedi.
## Lane GATE10

Gerçek: Kapı 10 (spec 6.12, `bench/README.md` "Local model measurement (gate 10)") ilk kez sentetik değil gerçek girdiyle koştu. `local.smart` `<vault>\.oom\oom.json` içinde `qwen3:14b` diyor; ölçüm makinesinde kurulu değildi (`qwen3:8b`, `qwen3:30b-a3b-instruct-2507-q4_K_M`, `odena-8b:latest` kuruluydu). Canlı kasanın `oom.json`'ı hiç düzenlenmedi — yalnız okundu; ölçüm bunun yerine kasanın bir KOPYASI üzerinde koştu: `.oom\oom.json` kopyalanıp yalnız `backend.local.smart` kurulu `qwen3:30b-a3b-instruct-2507-q4_K_M` olarak değiştirildi, izole `claude-config` kimlik dizini (`.credentials.json` dahil) ayrı bir kopya olarak taşındı, `knowledge\` gerçek kasaya salt-okunur bir junction ile bağlandı. Model uyuşmazlığı sessizce değiştirilmedi: rapor edilen model, gerçekten çağrılan modeldir.

`dotnet publish src/Oom/Oom.csproj -c Release -o publish/win-x64` sonrası dört koşum: (1) kuru koşum `.brief/gate10-dry.txt` — 30 transkript + 5 daily doğru çözüldü, hiçbir model çağrılmadı, hiçbir dosya yazılmadı; (2) `--judge` olmadan ilk gerçek geçiş (`bench/results/gate10-2026-09-10-akis-derleme-yalniz.json`) — yalnız zaman/kararlılık kontrolü için; (3) asıl ölçüm, üç ayağın tamamı tek komutta:

```
oom --vault <vault-copy> bench --backend local --transcripts 30 --dailies 5 --transcript-dir <user>\.claude\projects --daily-dir <vault>\daily --judge --out bench/results/gate10-2026-09-10.json
```

| ayak | n | sonuç | eşik | geçti |
| --- | ---: | ---: | ---: | --- |
| (a) flush beş bölümlü şekil | 30 | 0,400 | 0,95 | hayır |
| (b) çift-kör yargı | 2 | 2,00 | 3,50 | hayır |
| (c) compile uyumu | 5 | 0,000 | 0,95 | hayır |

Karar: `backend.flush = drop`, `backend.compile = drop`. Ham sonuç `bench/results/gate10-2026-09-10.json`.

Leg (a)'nın 0,400'ü yanıltıcı: 30 dosyadan 14'ü alt-ajan izi (`agent-*.jsonl`, spec 6.3-1 gereği sıfır özetlenebilir tur — `Bench`, `ingest`'in Y-112 alt-ajan filtresini uygulamıyor) ve 3'ü `NoNewTurns`; bu 17 dosya modele hiç gitmedi. Modele giden 13 transkriptten 12'si beş bölümlü şekli geçti, 1'i (bu oturumun kendi canlı transkripti) şekil doğrulamasını geçemedi — gerçek oran 12/13 = 0,923, hâlâ 0,95 eşiğinin altında ama ham 0,400'den çok farklı bir tablo.

Leg (b) yalnız 2 çift üzerinde koştu, 30 değil: `--judge`, yerel arka ucun 13 çağrılabilir özetini aynı 13 transkriptin taze bir `claude` referans özetiyle eşleştirip Claude `smart` yargıcına çift-kör puanlattı; yalnız 2 çiftte HER İKİ tarafta da boş olmayan cevap vardı (`RunJudge` boş cevaplı çiftleri sessizce atlıyor). Aynı 30 dosya + 5 daily girdisi üzerinde ayrı bir tanı koşumu (`oom --vault <vault-copy> bench --backend claude ...` → `bench/results/gate10-2026-09-10-claude-backend.json`) neden 13'ün çoğunun referans tarafında kullanılabilir cevap üretmediğini göstermek için başlatıldı; bu tanı koşumu rapor yazılırken (~12 dakika, sıralı `claude -p` çağrıları nedeniyle) hâlâ sürüyordu — bitmişse dosya kendi adında duruyor, bitmemişse bu rapor bunu "ölçülemedi" olarak işaretliyor (aşağıya bakın). `Table()`'ın konsola bastığı `n` sütunu (b) satırında `flushCount` (30) yazıyor, gerçek puanlanan çift sayısını değil — JSON'daki `"why": "2 çift..."` doğru, konsol tablosu kozmetik olarak yanıltıcı; bu üründe düzeltilmedi (kapsam dışı, rapor ediliyor). Puanın kendisi (2,00/5,00, eşik 3,50) gerçek bir çift-kör sonuç ama genellemeye çok ince bir örneklem.

Leg (c) 5/5 kaldı, hem de hiçbiri artık bir sözleşme ihlali değil: 4 daily HTTP 400 "bağlam uzunluğunu aşıyor" (sunucunun bildirdiği 11.534–21.151 token) ile reddedildi, beşincisi (en büyük daily) ilk geçişte Ollama `llama-server` alt sürecini `0xc0000409` ile tamamen çökertti (HTTP 500), yeniden denemede aynı bağlam-aşımı 400'ünü verdi. `qwen3:30b-a3b-instruct-2507-q4_K_M`'in kendi bağlam penceresi 262.144 token (`ollama show`); kusur modelin sınırı değil, `Runner.CallLocal` (`src/Oom/Runner/Runner.cs`) `/chat/completions` gövdesinde `options.num_ctx` hiç göndermiyor, bu yüzden Ollama örneği yüklendiği (çok daha küçük) bağlamda kalıyor — gerçek bir daily + gerçek kök harita + 400'e kadar kayıt defteri adı bunu rahatça aşıyor. Gerçek, raporlanabilir bir yerel-arka-uç kısıtı; bu şeritte ürün değişikliği yapılmadı (spec 6.12 zaten kargo yolunu ölçüyor, ölçtüğü kodu değil).

Kasaya hiçbir şey yazılmadı — ölçüldü, iddia edilmedi: `.brief/gate10-vault-once.txt` (önce) / `-sonra.txt` (sonra) `<vault>\daily`'nin 189 dosyasını bayt bayt, satır satır özdeş gösteriyor; `.oom` üst düzey listesinde yalnız `claude-config` dizininin mtime'ı değişti (kasadaki bağımsız arka plan `ingest`/sweep kancaları — bu şeridin başından beri paralel çalışıyordu — bizim ölçümümüz o dizine kopyalama dışında hiç dokunmadı), `oom.json`/`vault.json`/`hub-config.json`/`oom.exe` bayt bayt aynı. Canlı kasanın kendi state DB'si (`%LOCALAPPDATA%\oom\<vault-hash>\state.db`, +12 KB) değişti ama bu da aynı arka plan kancalarından: geçici kasa kopyasının KENDİ hash'i için `%LOCALAPPDATA%\oom\` altında HİÇBİR dizin hiç oluşmadı — `Bench`in `Flush`/`Compile`'ı state'siz kurduğu iddiası (`README`) doğrudan doğrulandı, uydurulmadı.
...
Gerçek: `<vault-v2>`'ye eski (dondurulmuş) kasadan `daily/` (182 dosya), `knowledge/` (554 kavram notu) ve Companion klasörü kopyalanıp temiz bir `oom install` çalıştırılmıştı. `oom doctor` bu yüzden `daily · uyarı · pending · 182` ve `doctor · hata · coverage · 7d · %5,7` (+ `sweep · kapsama-tum-zamanlar · 18/2019`) veriyordu — ikisi de doğru aritmetik, yanlış popülasyon üzerinde: `Program.Pending()` `daily_ingest`'te hiç iz olmadığı için zaten derlenmiş 182 dosyayı da kuyruğa koyuyordu; `SweepRun.Coverage()` ise `sweep.roots` altındaki HER `*.jsonl`'i (canlı makinede 2019 dosya, çoğu vault'tan aylar önce, ilgisiz projelerden) kapsama paydası sayıyordu. Ölçüm notu `.brief/olcum-ne.md`.

Y-117: `install --adopt` eklendi (`Program.AdoptCompiledVault`) — `knowledge/` kavram notlarının en yeni `created`/`updated` damgasını bulur, o damgaya EŞİT VEYA ÖNCEKİ tarihli her `daily/*.md`'yi `daily_ingest`'e `status='adopted'` ile yazar (gerçek `ingested`'tan ayrı, `Pending()` artık `status IN ('ingested','adopted')` hariç tutuyor) ve vault'un kendi `installed_at` damgasını (`vault_meta` tablosu) yoksa yazar. Hiçbir şey silinmez; zorla yeniden derleme hâlâ mümkün (`daily_ingest` satırı elle/`--fix` ile geri alınabilir). Test: `KurulumScars.Y117_AdoptionMarksAlreadyCompiledDailiesWithoutRequeuing` (3 daily + eşleşen kavram notu → 3'ü de adopted, damgadan sonraki 4. daily bekler).

Y-118: `SweepRun.Coverage()` artık `since` (vault'un `VaultInstalledAt()` damgası) parametresi alıyor; hem 7 günlük hem tüm-zamanlar penceresi bu damgadan ÖNCEKİ dosyaları hariç tutuyor, onları ayrı bir `sweep · arsiv · vault-oncesi` `bilgi` satırında raporluyor — asla `hata` değil. `Doctor.Check()`'teki `coverage` satırı da artık üç değerli: oran ≥ %95 → `bilgi`, oran düşük ama pencere küçükse (`WindowTotal < 5`) → `uyarı` (sakin hafta, mekanizma arızası değil), yalnız gerçek bir popülasyon üzerinde düşükse → `hata`. Test: `SurecIsletmeScars.Y118_CoverageMeasuresTheVaultsOwnWindow` (bir vault-öncesi + bir vault-sonrası transkript → yalnız sonraki sayılır, öncesi arşiv, seviye `hata` değil).

Kırmızı/yeşil: `.brief/red-mig.txt` (yeni testler `State.WriteVaultStamp`/`VaultInstalledAt`/`DoctorSnapshot.WindowTotal` henüz yokken derleme hatasıyla kırmızı — testin gerçekten yeni API'yi sürdüğünün kanıtı), `.brief/green-mig.txt` (133/133/0). Öncesi 131/131/0.

Satır bütçesi: `KurulumScars.Y068` tavanı (9.100) tam sınırda duruyordu, sıfır boşluk vardı. Yeni kod aşırı yoğunlaştırıldı (tek satırlık ifadeler, birleşik `HealthItem` satırları) ve `Compile.cs`'teki tamamen ölü, hiç çağrılmayan/test edilmeyen `DailyCandidate`/`SelectDailies` (spec'te tanımlı ama üretimde hiç bağlanmamış, tek commit'te doğmuş) kaldırıldı — bu, yeni davranışın gerektirdiği satırları dengeledi. Sonuç tam 9.100/9.100, `Y068` yeşil.

Canlı kanıt (`<vault-v2>`, kasaya yalnız `state.db` yazıldı; `daily/`, `knowledge/`, Companion dosyalarından hiçbiri silinmedi/yeniden yazılmadı; dondurulmuş eski kasa hiç açılmadı): `dotnet publish` sonrası `oom --vault <vault-v2> doctor` → `daily · uyarı · pending · 182 · Bekleyen daily: 182` ve `doctor · hata · coverage · 7d · Son 7 gün kapsama: %5,7` (`.brief/doctor-once.txt`). `oom --vault <vault-v2> install --adopt` → `benimseme: 182 daily zaten derlenmiş olarak işaretlendi (adopted)`. Sonra aynı `doctor` → `daily · bilgi · pending · 0 · Bekleyen daily: 0` (`.brief/doctor-sonra.txt`) — canlı, anlık, sweep beklemeden. `coverage`/`kapsama-tum-zamanlar` satırları bu ikinci okumada henüz DEĞİŞMEDİ: o sayı `coverage` tablosundaki EN SON `sweep` satırından okunuyor ve yeni ikili henüz gerçek bir `sweep` koşturmadı — bilerek koşturulmadı, çünkü canlı `sweep.roots` (`%USERPROFILE%\.claude\projects`, `.codex\sessions`) makine genelinde YİRMİ yeni oturumu gerçek `claude`/`local` arka ucuna göndermek ve bugünün daily'sine gerçek içerik eklemek anlamına geliyordu (`--dry-run` kanıtı: 20 aday, 4 "ok" olurdu) — bunu görevin "hiçbir daily/knowledge/Companion dosyası silinmez/yeniden yazılmaz" sınırının ruhunun dışında, gereğinden fazla bir yan etki olarak değerlendirip atladım. Y-118'in doğruluğu izole testle (yukarıda) kanıtlı; canlı `coverage` sayısı bir sonraki doğal `sweep` koşumunda (zamanlanmış görev) kendiliğinden düzelecek. Dondurulmuş eski kasa bu şerit boyunca hiç açılmadı.

Doğrulanamayan: Canlı vault'ta `coverage`/`kapsama-tum-zamanlar` satırlarının düzeltilmiş hâli — gerçek bir `sweep` koşturmadan görülemez (yukarıda gerekçeli).
