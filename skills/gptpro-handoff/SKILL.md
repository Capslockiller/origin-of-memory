...
<!-- ADAPTATION NOTES — the body below is Avenox's original
     (upstream github.com/avenoxai/avenoxskills, skills/gptpro-handoff/SKILL.md)
     with operator context filled in. references/prompt-style.md needed no
     changes and is byte-identical to upstream. -->

## ADAPTED: operator context

- **The deep model is a chat-UI frontier model on the operator's own
  subscription.** Bundles are uploaded into a Project so the snapshot persists
  across prompts in a round.
- **Every operator step in the loop — pasting the prompt into the web UI,
  choosing the reasoning tier, uploading zips, bringing the report back — is the
  operator's step and is never automated by an agent.** An agent authors the
  prompt into a file and says "ready to paste"; it does not open a browser and
  it does not drive the chat UI. Moving a codebase to a third party is a human
  decision each time.
- **The bundles come from the `gptpro` skill in this same set**, run under Git
  Bash. That port replaces `zip`/`rsync` where they are absent; the secret-scan
  HARD abort is preserved byte-for-byte and is what makes an upload safe.
- **`gptpro/` artifact folder** — the body puts prompts and reports in a
  gitignored `gptpro/` at the repo root. Keep that. Add `gptpro/` to the target
  repo's `.gitignore` before the first write; these files quote source and
  findings and must not be committed.
...
1. **Steps 4 and 5 are human handoffs, full stop.** See above.
2. **Step 6, VERIFY, is the gate and cannot be delegated away.** The body already
   says the conclusion is yours; the policy adds that a verification sub-agent
   returns evidence, not a verdict, and that its brief must carry the same
   OWNS / DO-NOT-TOUCH contract as any other lane. Sub-agent ceiling and model
   choice come from `orchestration`, not from this file.
3. **Step 7, IMPLEMENT, is direction-setting.** A returned roadmap is an external
   model's opinion about what this codebase should become. Confirm the roadmap
   with the operator before implementing from it — the body's "work the
   prioritized roadmap top-down" is the method, not the authorisation.
4. **Reporting follows the policy**: summarize into the main loop in the
   operator's language with sources named. The `.report.md` is a transcript, not
   the report.
5. ⚠ **"~50–60 minutes" per answer and "the large majority of findings hold up"
   are the upstream author's claims, unverified here.** Treat both as
   expectations to calibrate against your own first rounds, not as facts.
...
4. HAND OFF   — operator pastes the prompt into the web UI.     [OPERATOR, never automated]
...
live code, not the sub-agent's say-so. (Ceiling, model tier, and brief format
come from `orchestration`.)
