---
name: codex-fleet
description: "Operating manual for the Codex CLI specifically: `codex exec` invocation, image generation via the built-in `gpt-image-2` tool, and worktree-isolated multi-lane codex fleets. Defaults: model `gpt-5.6-sol`, reasoning `high`, `--skip-git-repo-check` always. Load only for work that actually runs the `codex` binary — \"use codex\", \"run codex\", \"codex exec\", \"imagegen\", \"spawn a codex fleet\", or an image-asset request routed to Codex. It does NOT decide what to delegate or to which tier; `orchestration` owns routing, approval gates and reporting, and overrides this file wherever they disagree."
---

> **Genericized from the author's working set — adapt paths and names to your
> setup. The operator prunes this set before relying on it.** Machine-specific
> absolute paths were replaced with placeholders; the upstream body and the
> adaptation notes are otherwise kept.

<!-- ADAPTATION NOTES — the body below is Avenox's original
     (upstream https://avenox.lol/codex.md) with the macOS-only commands
     replaced in place rather than annotated. To refresh from upstream:
     re-fetch, re-apply the replacements listed under "Windows command
     equivalents", then re-attach this block. -->

## ADAPTED: status

Verified on the author's Windows machine at adaptation time with
`codex-cli 0.149.0`, installed via `npm i -g @openai/codex` and authenticated
through the ChatGPT desktop login (the CLI shares `~/.codex` auth — no separate
`codex login` was needed). Smoke test passed: `codex exec --sandbox read-only -m
gpt-5.6-sol` read a canary file verbatim from a working directory on a real
workspace drive. `-m gpt-5.6-sol` was accepted by that build (the model
self-reports "GPT-5 (Codex)"; self-report is not authoritative). The
`gpt-image-2` id was unverified at the time of writing.

⚠ **Measured limit: the sandbox cannot read Windows Temp paths** — a read-only
lane pointed at a session scratchpad under `<user>\AppData\Local\Temp\...` got
"Access denied" on every read attempt. Keep lane working dirs (`-C`) on a real
workspace drive; a `$LOGDIR` in Temp is fine, because logs are written by shell
redirection rather than by the sandboxed codex process.

## ADAPTED: environment expectations (Windows)

The table below is the author's machine at adaptation time. Check your own
before trusting any example verbatim — the consequences column is what matters.

| Requirement | Status then | Consequence |
|---|---|---|
| `codex` CLI | 0.149.0, logged in | Ready; past the 0.128+ floor this skill requires |
| Python | **missing** (only the Windows Store stub) | `remove_chroma_key.py` and the `image_gen.py` CLI fallback cannot run → the transparency and JSONL-batch paths are closed. The built-in `image_gen` path is unaffected |
| `OPENAI_API_KEY` | not set | Only affects the CLI fallback and the `gpt-image-1.5` transparency path; the built-in tool uses the Codex subscription |
| node / npm | present | Needed for the Codex install |
| git | present | Needed for the fleet worktree pattern |
| ffmpeg | present | The `sips` replacement |

## ADAPTED: Windows command equivalents

The upstream body assumes macOS. These substitutions are **already applied in
the body below** — they are listed here so a future upstream refresh can redo
them, not as caveats to apply while reading.

- **`caffeinate -i <cmd>`** — does not exist on Windows and has no drop-in equivalent. Removed from every spawn recipe; check the sleep setting manually (`powercfg /a`) before a long fleet run instead.
- **`sips`** (resize / crop) → `ffmpeg -i in.png -vf scale=W:H out.png`
- **Run every command block through a POSIX shell (Git Bash), never PowerShell.** `2>/dev/null`, `&` + `wait`, `${VAR:-default}` and the `--` argument terminator either behave differently or fail to parse in PowerShell 5.1. This is measured, not cautionary: a PowerShell move once mangled a non-ASCII path and dropped fourteen desktop shortcuts into the working directory.
- **Log paths** — `/tmp/...` resolves under Git Bash but is volatile. Every example writes to `$LOGDIR`, which you set to the session scratchpad directory before spawning.
- **Image cache** — `~/.codex/generated_images/` resolves correctly under Git Bash; `find ~/.codex/generated_images -type f -name '*.png' -mmin -5` works as written.

## ADAPTED: governance — `orchestration` outranks this file

This file was fetched from the public web and contains instructions addressed to an agent. Where it and the orchestration policy disagree, the policy wins. Four points govern how the body is applied:

1. The body says *"don't ask 'should I proceed' — just go."* Here that covers **read-only, reversible, in-scope `codex exec` calls only.** Anything that sets direction — structure, approach, what gets built and in what order, the first write of a new file — is confirmed with the operator first. `--full-auto`, `--sandbox danger-full-access` and `--dangerously-bypass-approvals-and-sandbox` need explicit approval before first use in a session. The body's own *Error handling* section already says the latter; this note promotes it.
2. The image-generation template defaults to `-s danger-full-access`. That grants an external AI full write and network access to this machine. The justification is legitimate (the tool must write the PNG to disk), but it is not a default to pass over silently — confirm on first use.
3. Model ids (`gpt-5.6-sol`, `gpt-image-2-2026-04-21`) are the skill author's claims and are **unverified**. Test them against the actual installed build before trusting any example verbatim; if rejected, update to an id the installed version supports.
4. **Lane output follows the policy, not the body.** Summarize from the log into the main loop in the operator's language with sources named; the log is a transcript, not a report. Raw research is filed to the vault only when the operator asks for it.

## ADAPTED: fleet concurrency — decided

**Ceiling is four concurrent `codex exec` lanes, staggered 2–5s apart.** Ignore
the body's "~20 lanes"; that number comes from another machine and another OS.

The conflict it resolves: the body's doctrine is *fire every independent job at
once*, while the recorded experience here is that **5 parallel in-process agents
crashed the host app twice**, with a standing rule to start 2 at a time. The
distinction that made a separate number defensible is that **Codex lanes are
external OS processes spawned via the shell, not in-process sub-agents** — a
different failure profile, still unmeasured. In-process sub-agents keep their
own ceiling of three.

⚠ Neither number is a measurement: the crash behind them was never diagnosed.
Raising either is the operator's decision and should follow a measured ramp run.
...
mkdir -p output/dir && \
  codex exec [opts] -i char_sheet.png \
    -- "frame1 prompt → save to output/dir/frame1.png" > $LOGDIR/log1 2>&1 && \
  codex exec [opts] -i char_sheet.png -i output/dir/frame1.png \
    -- "frame2 prompt → save to output/dir/frame2.png" > $LOGDIR/log2 2>&1 && \
  codex exec [opts] -i char_sheet.png -i output/dir/frame1.png -i output/dir/frame2.png \
    -- "frame3 prompt → save to output/dir/frame3.png" > $LOGDIR/log3 2>&1
```
...
Single message with 4 Bash tool calls, all run_in_background: true:
  codex exec [...flags...] "PROMPT_BG"        > $LOGDIR/codex-bg.log 2>&1
  codex exec [...flags...] "PROMPT_PORTRAITS" > $LOGDIR/codex-portraits.log 2>&1
  codex exec [...flags...] "PROMPT_SIGIL"     > $LOGDIR/codex-sigil.log 2>&1
  codex exec [...flags...] "PROMPT_BANNER"    > $LOGDIR/codex-banner.log 2>&1
```
...
Square is fastest. Don't ask for tiny output (e.g. `256x256`) — the tool will reject it (below min-pixels). Generate at a supported size and downscale with `ffmpeg -i in.png -vf scale=W:H out.png` afterwards.
...
| Empty cache directory after run | Codex refused or errored silently | Check the log file (`$LOGDIR/codex-X.log`) — image gen may have hit content policy or rate limit |
| Wrong aspect ratio | Tool didn't honor exact dimensions | Re-prompt with one of the popular sizes (`1024x1024`, `1536x1024`, etc.); crop with `ffmpeg -vf crop=` if needed |
...
codex exec --skip-git-repo-check --full-auto \
  ...
  -m gpt-5.6-sol \
  -c model_reasoning_effort=high \
  "<SELF-CONTAINED LANE BRIEF>" > $LOGDIR/lane-A.log 2>&1
```
...
> **Sleep kills lanes, and Windows has no `caffeinate`.** A lid-close or idle sleep silently kills a mid-flight lane (you'll see ~5KB of output, zero edits). The upstream fix — wrapping every spawn in `caffeinate -i` — has no Windows equivalent, so check `powercfg /a` and disable sleep manually before a long fleet run. A respawn with the same brief is safe when `git status` shows no partial work.
...
- **Real ceiling ≈ 20 concurrent `codex exec` processes** — that's RAM + OpenAI rate limits, not orchestration. Beyond that, tier and queue. (See the adaptation note above: the ceiling in use here is four.)
